...
/*   Updated: 2022/06/03 00:26:03 by kkaneko          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char		*buf;
	static char	*save[FD_MAX + 1] = {NULL};
	ssize_t		read_bytes;
	...
	while (save[fd] != NULL)
	{
		...
		if (read_bytes < BUFFER_SIZE && !(ft_strchr(buf, '\n')))
			return (ft_hstrjoin_s(&save[fd], &buf));
		save[fd] = ft_hstrjoin_s(&save[fd], &buf);
	}
	return (NULL);
}
